...
export function KioskIdle({ session }: KioskIdleProps) {
  ...
  const qrUrl = session?.id 
    ? `https://api.qrserver.com/v1/create-qr-code/?size=210x210&data=${encodeURIComponent(`http://10.80.4.130:3000/?session=${session.id}&mode=mobile`)}`
    : '';
    ...
    >
      ...
      <div className="relative z-10 flex items-center justify-between px-12 py-7">
        ...
        <div className="flex items-center gap-2" style={{ color: '#475569' }}>
          <Wifi size={16} />
          <span style={{ fontSize: '0.85rem', fontFamily: 'JetBrains Mono, monospace' }}>KIOSK-001</span>
        </div>
      </div>
      ...
      <div className="relative z-10 flex-1 flex flex-col items-center justify-center gap-6">
        <div className="relative flex items-center justify-center">
          {[0, 1, 2].map((i) => (
            <motion.div
              ...
              transition={{ repeat: Infinity, duration: 2.8, delay: i * 0.75, ease: 'easeOut' }}
            />
          ))}
          ...
          >
            {qrUrl ? (
              <a href={qrUrl} target="_blank" rel="noopener noreferrer">
                ...
                >
                  <img src={qrUrl} alt="QR Code" width={210} height={210} style={{ borderRadius: '8px', display: 'block' }} />
                  {[
                    ...
                    <div key={i} className={`absolute -m-2 w-5 h-5 ${cls}`} style={{ borderColor: '#2563EB' }} />
                  ))}
                </div>
              </a>
            ) : (
              ...
              >
                <span style={{ color: '#0F172A' }}>Cargando QR...</span>
              </div>
            )}
          </motion.div>
        </div>
        ...
        >
          <h1 style={{ color: '#F1F5F9', fontWeight: 800, fontSize: '2.6rem', letterSpacing: '0.02em' }}>
        ...
        >
          {[
            ...
            <div key={n} className="flex items-center gap-3 px-6 py-3 rounded-full" style={{ background: '#0A1628', border: '1px solid #1E3A6E' }}>
              <span style={{ color: '#2563EB', fontWeight: 700, fontFamily: 'JetBrains Mono, monospace', fontSize: '0.85rem' }}>{n}</span>
      ...
      <div className="relative z-10 flex items-center justify-between px-12 py-5" style={{ borderTop: '1px solid #0A1628' }}>
        ...
        <span style={{ color: '#334155', fontSize: '0.85rem', textTransform: 'capitalize' }}>{dateStr}</span>
        <span style={{ color: '#94A3B8', fontSize: '1.6rem', fontWeight: 300, fontFamily: 'JetBrains Mono, monospace' }}>{timeStr}</span>
      </div>
    </div>
  );
}